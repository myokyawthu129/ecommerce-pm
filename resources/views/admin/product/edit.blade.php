        ...
        <div class="col-8">
            <div class="card p-4">
                ...
                <div class="form-group">
                    ...
                    <img src="{{ asset('/images/'.$p->image) }}" style="wdith:150px" class="img-thumbnail" alt="">
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <select name="supplier_slug" class="w-100" id="supplier">
                        @foreach ($supplier as $s)
                            <option value="{{ $s->id }}" @if($s->id==$p->supplier_id)selected @endif>{{ $s->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    ...
                    <select name="category_slug" class="w-100" id="category">
                        @foreach ($category as $s)
                            <option value="{{ $s->slug }}" @if($s->id==$p->category_id) selected @endif>{{ $s->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    ...
                    <select name="brand_slug" class="w-100" id="brand">
                        @foreach ($brand as $s)
                            <option value="{{ $s->slug }}" @if($s->id==$p->brand_id)selected @endif>{{ $s->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    ...
                    <select name="color_slug[]" class="w-100" id="color" multiple>
                        @foreach ($color as $s)
                            <option value="{{ $s->slug }}" @foreach($p->color as $pc)
                                @if($pc->id==$s->id) selected @endif @endforeach>{{ $s->name }}</option>
                        @endforeach
                    </select>
                </div>
